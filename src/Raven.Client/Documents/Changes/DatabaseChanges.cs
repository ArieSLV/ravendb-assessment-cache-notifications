...
namespace Raven.Client.Documents.Changes
{
    ...
    {
        private static readonly IRavenLogger Logger = RavenLogManager.Instance.GetLoggerForClient<DatabaseChanges>();
        ...
        internal IChangesObservable<CacheInvalidationChange> ForAggressiveCaching()
        {
            var counter = GetOrAddConnectionState("aggressive-caching", "watch-aggressive-caching", "unwatch-aggressive-caching", null);

            var taskedObservable = new ChangesObservable<CacheInvalidationChange, DatabaseConnectionState>(
                counter,
                notification => true);
        ...
        private void NotifySubscribers(string type, BlittableJsonReaderObject value)
        {
            switch (type)
            {
                case CacheInvalidationChange.NotificationType:
                    if (CacheInvalidationChange.TryRead(value, out var cacheInvalidation, out string rejectionReason) == false)
                    {
                        if (Logger.IsWarnEnabled)
                            Logger.Warn($"Ignoring cache invalidation changes notification. Reason: {rejectionReason}");
                        break;
                    }

                    if (Logger.IsInfoEnabled)
                        Logger.Info($"Applying cache invalidation changes notification. Reason: {cacheInvalidation.Reason}. Generation: {cacheInvalidation.Generation}.");

                    foreach (var state in States.ForceEnumerateInThreadSafeManner())
                    {
                        state.Value.Send(cacheInvalidation);
                    }
                    break;
