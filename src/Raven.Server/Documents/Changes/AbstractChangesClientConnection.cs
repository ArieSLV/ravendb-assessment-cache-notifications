    ...
    where TOperationContext : JsonOperationContext
{
                ...
                while (true)
                {
                    ...
                    await using (var writer = new AsyncBlittableJsonTextWriter(context, ms))
                    {
                        ...
                        do
                        {
                            ...
                            if (CanWriteMessage(value) == false)
                                continue;

                            if (first == false)
                                writer.WriteComma();

                            if (TryWriteSpecialMessage(context, writer, value) == false)
                            {
                                switch (value)
                                {
    ...
    protected virtual bool CanWriteSpecialMessage(object message)
    {
        return false;
    }

    protected virtual bool TryWriteSpecialMessage(JsonOperationContext context, AsyncBlittableJsonTextWriter writer, object message)
    {
        return false;
    }

    private bool CanWriteMessage(object value)
    {
        return value is DynamicJsonValue or BlittableJsonReaderObject || CanWriteSpecialMessage(value);
    }
